# variables.tf - Input Variables for Redis Terraform Configuration
...
variable "container_memory_mb" {
  description = "Memory to allocate to the container in megabytes (default: 2GB = 2048 MB)"
  type        = number
  default     = 2048

  validation {
    condition     = var.container_memory_mb >= 512
    error_message = "Memory must be at least 512MB."
  }
}

variable "container_disk_gb" {
  description = "Disk size to allocate to the container in gigabytes (default: 10GB)"
  type        = number
  default     = 10

  validation {
    condition     = var.container_disk_gb >= 5
