...
output "container_vmid" {
  ...
  value       = proxmox_virtual_environment_container.redis.vm_id
}

output "container_hostname" {
  description = "Hostname assigned to the container"
  value       = proxmox_virtual_environment_container.redis.initialization[0].hostname
}

output "container_node" {
  description = "Physical Proxmox node running this container"
  value       = proxmox_virtual_environment_container.redis.node_name
}
...
output "next_steps" {
  ...
  value       = <<-EOT
    ...
    3. Create vault file with Redis password:
    ...
    5. Verify Redis is running:
       redis-cli -h ${local.network_config.ipv4 != null ? split("/", local.network_config.ipv4)[0] : "<CONTAINER_IP>"} ping
